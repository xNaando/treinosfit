...
await p.screenshot({ path: '.shots/ob1-inicio.png' })
...
await p.screenshot({ path: '.shots/ob2-medidas-gordo.png' })

// altura máxima — cabeça não pode ser cortada
await p.locator('input[type=range]').nth(0).fill('220')
await p.waitForTimeout(600)
await p.screenshot({ path: '.shots/ob2b-altura-max.png' })
await p.locator('input[type=range]').nth(0).fill('175')

await p.locator('input[type=range]').nth(1).fill('55')
await p.waitForTimeout(600)
await p.screenshot({ path: '.shots/ob3-medidas-magro.png' })
...
await p.screenshot({ path: '.shots/ob4-avatar-m.png' })
await p.click('text=Boné')
await p.waitForTimeout(500)
await p.screenshot({ path: '.shots/ob5-bone.png' })
await p.click('text=Moicano')
await p.waitForTimeout(500)
await p.screenshot({ path: '.shots/ob6-moicano.png' })
...
await p.click('text=Próximo')
await p.waitForTimeout(800)
await p.screenshot({ path: '.shots/ob7-avatar-f.png' })
await p.click('text=Maria-chiquinha')
await p.waitForTimeout(500)
await p.screenshot({ path: '.shots/ob8-chiquinhas.png' })
